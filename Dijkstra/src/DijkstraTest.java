...
public class DijkstraTest {
	...
	/**
	 * Loads the graph from the input file. Runs Dijktra's algorithm on every
	 * origin node and for each destination node, compares the returned path
	 * with the correct path in the resultsFile.
	 * 
	 * @param inputFile
	 * @param resultsFile
	 * @return
			...
			for (int i = 0; i < gr.numNodes(); i++) {
				...
				for (int j = 0; j < gr.numNodes(); j++) {
					if (j != i) {
						...
						ArrayList<Integer> path = dijkstra
								.shortestPath(destNode);
						String pathString = br.readLine();
						if (pathString == null) {
							System.out
									.println("Error reading from the resultsFile");
							return false;
						}
						...
						if (path.size() != expectedPath.length) {
							...
							System.out.println("Your path's size = "
									+ path.size());
							System.out.println("Expected the path of size: "
									+ expectedPath.length);
							DijkstraTest.printInfo(originCity, destCity, path,
									expectedPath, gr);
							return false;
						}
						assert (expectedPath[expectedPath.length - 1]
								.equals(destCity));
						// comparing the cities on the returned path and the
						// expected path
						for (int k = 0; k < path.size(); k++) {
							...
							if (!cityString.equals(expectedPath[k])) {
								DijkstraTest.printInfo(originCity, destCity,
										path, expectedPath, gr);
								return false;
							}
	...
	/**
	 * Helper function, used for printing the error messages when the test
	 * function finds the mismatch between the expected result and the returned
	 * result.
	 * 
	 * @param originCity
	 * @param destCity
	 ...
	 *            gr
	 */
	public static void printInfo(String originCity, String destCity,
			ArrayList<Integer> returnedPath, String[] expectedPath, MapGraph gr) {

		System.out.println("MISMATCH FOUND: For cities " + originCity + " -> "
				+ destCity);
