...
public class GUIApp extends JFrame {
	...
	class MapPanel extends JPanel {
		...
		public void drawGraph(Graphics g) {
			if (nodeLocations == null || nodeColors == null
					|| nodeLabels == null) {
				System.out
						.println("Can't display nodes - the array of node locations has not been initialized");
				return;
			}
		 ...
		 * (corresponding to the beginning and end vertices of the edge). Note
		 * that this representation is different from what is stored in the
		...
		class MyListener implements MouseListener {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (!oneClicked) {
					...
					System.out
							.println("Run Dijkstra's computePaths method for this source vertex");

					// call dijkstra.computePaths(v);
					...
					System.out
							.println("Call Dijkstra's shortestPath() method to compute the shortest path between selected cities");

					// Call: path = dijkstra.shortestPath(v);
