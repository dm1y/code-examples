/**
 * Run this file to test your cs245 Project 3 code. Keep in mind that while failing 
...
public class FlightListTester {
	...
	public boolean testFind(String filename) {
		...
		try {
			...
			while ((s = br.readLine()) != null) {
				...
				if (!list.find(key)) {
					passed = false;
					System.out.println("Failed to find the following flight: "
							+ s);
				}
			}
		} catch (IOException e) {
			System.out
					.println("IOException occured while reading from the file: "
							+ e);
			passed = false;
		}
		if (passed)
		...
		for (int i = 0; i < testedFlights.size(); i++) {
			...
			if (expectedResult.length != results.size()) {
				System.out.println(message1
						+ " the number of flights returned for " + key
	 ...
	 * flight nodes.
	 */
	public boolean compareToExpectedResult(FlightNode[] expectedResult,
			ArrayList<FlightNode> results, String message) {
	...
	public void prepareTestData() {
		...
		FlightNode[] expectedList1 = { new FlightNode(resKey1, data1),
				new FlightNode(resKey2, data2) };
		...
		FlightNode[] expectedList2 = { new FlightNode(resKey1, data1),
				new FlightNode(resKey2, data2), new FlightNode(resKey3, data3) };
		...
		FlightNode[] expectedList3 = { new FlightNode(resKey1, data1),
				new FlightNode(resKey2, data2), new FlightNode(resKey3, data3) };
		...
		FlightNode[] expectedList4 = { new FlightNode(resKey1, data1),
				new FlightNode(resKey2, data2) };
		...
		FlightNode[] expectedList5 = { new FlightNode(resKey1, data1),
				new FlightNode(resKey2, data2) };
	...
	public void prepareTestDataSuccessors() {
		...
		flightsToTestSuccessors.add(new FlightKey("FRA", "JFK", "01/03/2014",
				"05:50"));
		flightsToTestSuccessors.add(new FlightKey("FRA", "JFK", "01/03/2014",
				"16:30"));
		flightsToTestSuccessors.add(new FlightKey("FRA", "SFO", "01/03/2014",
				"06:00"));
		...
		FlightNode[] expectedList1 = { new FlightNode(resKey1, data1),
				new FlightNode(resKey2, data2), new FlightNode(resKey3, data3),
				new FlightNode(resKey4, data4) };

		// expected successors for the second test flight
		FlightNode[] expectedList2 = { new FlightNode(resKey3, data3),
				new FlightNode(resKey4, data4) };
	...
	public void prepareTestDataPredecessors() {
		...
		flightsToTestPredecessors.add(new FlightKey("FRA", "JFK", "01/03/2014",
				"05:50"));
		flightsToTestPredecessors.add(new FlightKey("FRA", "JFK", "01/03/2014",
				"16:00"));
		flightsToTestPredecessors.add(new FlightKey("FRA", "JFK", "01/03/2014",
